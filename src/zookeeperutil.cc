...
void ZKclient::Start()
{
    ...
    watcher回调线程
    */
    //使用前初始化，要连接的ip+port，回调函数，超时心跳时间，0代表临时节点：客户端超时就删除这个节点
    ...
    //此时不代表已经连接成功，只有等服务端调用回调函数才是连接成功，所以sem等待
    sem_t sem;
    sem_init(&sem,0,0);//0代表一个进程的内的多个线程使用这个信号量，初始化资源为0
    zoo_set_context(m_zhandle,&sem);//设置上下文，添加额外信息

    sem_wait(&sem);         //信号量-1.阻塞等待sem+1
