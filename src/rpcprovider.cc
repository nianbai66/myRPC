...
void RpcProvider::Run()
{
    ...
    //绑定链接回调和消息读写回调方法,
    //    muduo库好处是很好的分离了网络代码和业务代码，只需要关注读写链接，和读写连接做的事
    ...
    //设置muduo库的线程数量，自动分配io线程（新用户得链接）和工作线程
...
/*
在框架内部需要提前协商好通信使用的protobuf数据类型：比如发送过来的数据类型为：service_name（对象）,method_name（method方法）,args
需要定义proto的message类型，进行数据头的序列化和反序列化，为防止TCP的粘包（当前得参数和下一个请求头连起来），需要对各个参数进行参数的长度明确

定义header_size（4字节，） + header_str（包含服务名和方法名，还有args得size） + args_str
...
void RpcProvider::OnMessge(const muduo::net::TcpConnectionPtr &conn,
                                    ...
                                    muduo::Timestamp)
{
    // //获取到数据，即网络上接受的远程rpc调用请求的字符流， Login和args。将网络中得数据转成string
    ...
    recv_buf.copy((char*)&header_size,4,0);             //从buf里0字符开始，拷贝4个字节，给headersizeUINT32类型得值，获他的char*来按地址copy
    ...
    // std::cout<<"======================================"<<std::endl;
    if(rpcHeader.ParseFromString(rpc_header_str))//开始对头对象进行反序列化，参数接受类型为引用,返回值为bool型
    {
        //数据头反序列化成功
    ...
    //生成rpc方法调用的请求request和相应response参数,请求和响应都继承自protobuf中得message类，
    google::protobuf::Message *request = service->GetRequestPrototype(method).New();//生成一个新对象，根据方法描述，服务能获取请求type
